...
def _evaluate(model, dataloader, class_names, img_size, iou_thres, conf_thres, nms_thres, verbose):
    ...
    for _, imgs, targets in tqdm.tqdm(dataloader, desc="Validating"):
        # Debugging to confirm tensors are on the correct device
        print(f"Images device before move: {imgs.device}")
        print(f"Targets device before move: {targets.device}")

        # Move images and targets to the same device as the model
        imgs = imgs.to(device)
        targets = targets.to(device)

        # Debugging to confirm tensors are on GPU
        print(f"Images device after move: {imgs.device}")
        print(f"Targets device after move: {targets.device}")
        ...
        with torch.no_grad():
            outputs = model(imgs)  # Model inference
